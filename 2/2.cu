...
void lookup(unsigned i,unsigned* cuda_S, unsigned* cuda_tables,unsigned keysize,unsigned tablesize, unsigned* cuda_secondtable,int hash_num){
		int blockSize = 64;
		int gridSize = (keysize + blockSize - 1) / blockSize; 
	    clock_t start, stop;
    	float costtime;

    	start = clock();
		lookupHash<<<gridSize,blockSize>>>(cuda_S, cuda_tables,keysize,tablesize, cuda_secondtable,hash_num);
		cudaDeviceSynchronize();
    	stop = clock();
    	costtime = ((double)(stop - start)*1000000 / CLOCKS_PER_SEC);
    	printf("i = %d, time = %.2f μs.\n", i,costtime);
}
...
int main() {
	for(unsigned t = 0; t < 5; ++t) {
		...
		cuckooHash<<<gridSize,blockSize>>>(cuda_tables, cuda_keys, keysize, M,hash_num,tablesize,secondtable);
		
		
		std::random_device rd1;
		std::mt19937 gen1(rd1());
		std::uniform_int_distribution<unsigned> dis1(1, keysize-1);

		for(unsigned i = 0; i <= 10; ++i) {
			unsigned *S = (unsigned *)malloc(keysize*sizeof(unsigned));
			for(unsigned j = 0; j < keysize; ++j) {
				if(j<(100-10*i)*keysize/100){
					S[j] = keys[dis1(gen1)];
				}else{
					S[j] = dis(gen);
				}
			...
			unsigned* cuda_S;
			cudaMalloc(&cuda_S, keysize*sizeof(unsigned));
			cudaMemcpy(cuda_S, S, keysize, cudaMemcpyHostToDevice);

			lookup(i,cuda_S, cuda_tables,keysize,tablesize, cuda_secondtable,hash_num);
